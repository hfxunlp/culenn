...
#define TAILLENSOFTMAX_THREADS 128

template <typename T, typename AccumT, typename IndexT>
__global__ void culenn_TailLenSoftMax_updateOutput_kernel(
  T *output, T *input, int nframe, int dim, IndexT *len)
{
  __shared__ AccumT buffer[TAILLENSOFTMAX_THREADS+1];
  ...
  if (threadIdx.x == 0)
  {
    ...
    buffer[TAILLENSOFTMAX_THREADS] = max_k;
  }

  __syncthreads();

  // sum?
  T max_k = ScalarConvert<AccumT, T>::to(buffer[TAILLENSOFTMAX_THREADS]);
  ...
  if (threadIdx.x == 0)
  {
    ...
    for (int i=0; i<blockDim.x; i++)
      sum_k += buffer[i];
    buffer[TAILLENSOFTMAX_THREADS] = sum_k;
  }

  __syncthreads();

  // softmax
  T sum_k = ScalarConvert<AccumT, T>::to(buffer[TAILLENSOFTMAX_THREADS]);
  for (int i=i_start; i<i_end; i+=i_step)
    output_k[i] = output_k[i] / sum_k;
}

template <typename T, typename AccumT, typename IndexT>
__global__ void culenn_TailLenSoftMax_updateGradInput_kernel(
  T *gradInput, T *output, T *gradOutput, int nframe, int dim, IndexT *len)
{
  __shared__ AccumT buffer[TAILLENSOFTMAX_THREADS];
