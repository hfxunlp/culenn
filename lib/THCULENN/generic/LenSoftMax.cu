           ...
           THCIndexTensor *len)
{
  ...
  culenn_LenSoftMax_updateOutput_kernel<real, accreal, THCIndex_t><<<blocks, threads, 0, THCState_getCurrentStream(state)>>>(
           ...
           THCIndexTensor *len)
{
  ...
  dim3 blocks(batchSize, blocksY, blocksZ);
  dim3 threads(LENSOFTMAX_THREADS);
  culenn_LenSoftMax_updateGradInput_kernel<real, accreal, THCIndex_t><<<blocks, threads, 0, THCState_getCurrentStream(state)>>>(
